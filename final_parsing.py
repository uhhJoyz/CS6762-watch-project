...
def main(output_file="features.csv", extra_features=False, window=1):
    lock_folder = "./door_data/lock/"
    non_lock_folder = "./door_data/negative/"
    ...
    with open(output_file, "w", newline="") as file:
        ...
        for file_name in os.listdir(lock_folder):
            file_path = os.path.join(lock_folder, file_name)
            process_file(
                file_path,
                "lock",
                writer,
        ...
        for file_name in os.listdir(non_lock_folder):
            file_path = os.path.join(non_lock_folder, file_name)
            process_file(
                file_path,
                "no_lock",
                writer,
...
if __name__ == "__main__":
    for i in range(1, 4):
        main(f"features_{i}.csv", extra_features=True, window=i)
